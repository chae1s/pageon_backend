...
@Transactional
@ActiveProfiles("test")
@DisplayName("CreatorWebtoonService 단위 테스트")
@ExtendWith(MockitoExtension.class)
class CreatorWebtoonServiceTest {
    @InjectMocks
    private CreatorWebtoonService webtoonService;
    @Mock
    ...
    void createContent_withValidCreatorAndCorrectInput_shouldCreateWebtoon() {
        ...
        ContentCreateRequest request = new ContentCreateRequest("웹툰 제목", "웹툰 설명", "하나,둘,셋,넷", mockFile, "MONDAY");
        List<Keyword> keywords = new ArrayList<>();

        doReturn(keywords).when(keywordService).separateKeywords("하나,둘,셋,넷");

        // 파일 업로드 했다고 가정
        doReturn("/webtoons/cover.png").when(fileUploadService).upload(any(), anyString());
